...
resource "aws_cloudwatch_metric_alarm" "cache_cpu" {
  count               = module.this.enabled && var.cloudwatch_metric_alarms_enabled && var.alarm_cpu_threshold_percent != 0? local.member_clusters_count : 0
...
resource "aws_cloudwatch_metric_alarm" "cache_swapusage" {
  count               = module.this.enabled && var.cloudwatch_metric_alarms_enabled && var.alarm_swapusage_threshold_bytes != 0 ? local.member_clusters_count : 0
  alarm_name          = "${element(local.elasticache_member_clusters, count.index)}-swapusage"
  alarm_description   = "Redis cluster swap usage"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "1"
  ...
  threshold = var.alarm_swapusage_threshold_bytes
